...
{
    protected $helper;
    protected $stripeRepository, $coinPayment, $deposit;
    ...
    public function create(Request $request)
    {
        ...
        $data['defaultWallet']      = Wallet::where(['user_id' => auth()->user()->id, 'is_default' => 'Yes'])->value('currency_id');
    ...
    public function confirm(Request $request)
    {
        ...
        $currency                   = Currency::where(['id' => $currency_id, 'status' => 'Active'])->first(['symbol', 'code']);
        ...
        if ($payMtd->name == 'Bank')
        {
            ...
            return view('user.deposit.bank', $data);
        }
        if (config('mobilemoney.is_active') && $payMtd->name == 'MobileMoney') {
            ...
            return view('user.deposit.mobilemoney_confirmation', $data);
        }
    ...
    public function store(Request $request)
    {
        ...
        if ($method == 'Paypal')
        {
            ...
            return view('user.deposit.paypal', $data);
        }
        ...
        else if ($method == 'Coinpayments')
        {
            ...
            if ($currency->type == 'crypto') {
                ...
                if (! empty($currency->code) && in_array($currency->code, $acceptedCoinIso)) {
                    ...
                    $transactionData = [
                        ...
                        'buyer_name' => getColumnValue(auth()->user()),
                        ...
                        if ($transactionInfo['error'] === 'ok') {
                            ...
                            if (isset($makeTransaction['payload']['type']) && $makeTransaction['payload']['type'] == "deposit") {
                                ...
                                try
                                {
                                    ...
                                    return view('user.deposit.coinpayment_summery', ['transactionDetails' => $makeTransaction, 'transactionInfo' =>  $transactionInfo]);
                                }
                                catch (Exception $e)
                                {
                                    DB::rollBack();
            ...
            return view('user.deposit.coinpayment', $data);
        }
    ...
    public function stripePayment()
    {
        ...
        return view('user.deposit.stripe', $data);
    }
        ...
        try {
            ...
            if (module('Referral') && settings('referral_enabled') == 'Yes') {
                $refAwardData = [
                    ...
                    'presentAmount'   => $depositConfirm['transaction']->subtotal,
    ...
    public function payumoneyPayment()
    {
        ...
        $amount = session('transInfo')['totalAmount'];             //fixed - was getting total - should get amount
        ...
        return view('user.deposit.payumoney', $data);
    }
            ...
            try
            {
                ...
                if (module('Referral') && settings('referral_enabled') == 'Yes') {
                    $refAwardData = [
                        ...
                        'presentAmount'   => $sessionValue['amount'],
                        'paymentMethodId' => $sessionValue['payment_method'],
                        'transactionType' => 'Deposit',
                    ];
    ...
    public function viewCoinpaymentTransactionInfo()
    {
        ...
        return view('user.deposit.coinpayment_summery', $data);
    }
                ...
                if (isset($coinLogResponse->deposit_id) && $coinLogResponse->type == 'deposit') {
                    ...
                    if ($status == 100 && $status_text == 'Complete') {
                        ...
                        try {
                            ...
                            // Notification Email/SMS
                            (new NotifyAdminOnDepositMailService)->send($merchantPayment, ['type' => 'deposit']);
    ...
    public function payeerPayement()
    {
        ...
        return view('user.deposit.payeer', $data);
    ...
    public function bankPaymentConfirm(Request $request)
    {
        ...
        try {
            ...
            $data['transaction'] = $depositConfirm['transaction'];
            return \Redirect::route('user.deposit.bank.success')->with(['data' => $data]);
        } catch (Exception $e) {
            DB::rollBack();
            session()->forget(['coinpaymentAmount', 'wallet_currency_id', 'method', 'payment_method_id', 'amount', 'transInfo']);
            clearActionSession();
