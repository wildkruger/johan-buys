...
<div class="section-payment">
    <div class="transaction-details-module"> 
        ...
        <div class="transaction-order-quantity">
            ...
            <p>{{ __('Order ID') }}: #{{ $paymentInfo['order_no'] ? $paymentInfo['order_no'] : "" }}</p>
            @if ($feeBearer == 'User')
        ...
        <div class="transaction-total d-flex justify-content-between mb-30">
            <h3>{{ __('Total') }} ({{ optional($merchant->currency)->code }})</h3>
            <span>{{ formatNumber($totalAmount, optional($merchant->currency)->id) }}</span>
        </div>
        <form action="{{ url('payment/mts_pay') }}" method="post" id="walletPaymentForm">
            @csrf
            <input name="merchant" value="{{ isset($paymentInfo['merchant_id']) ? $paymentInfo['merchant_id'] : '' }}" type="hidden">
            <input name="amount" value="{{ isset($paymentInfo['amount']) ? $paymentInfo['amount'] : '' }}" type="hidden">
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="form-group mb-3">
                        <label class="form-label" for="password">{{ __('Password') }} <span class="star">*</span></label>
            ...
            <div class="d-grid">
                <button class="btn btn-lg btn-primary" type="submit" id="walletSubmitBtn">
                    ...
                    <span id="walletSubmitBtnText" class="px-1">{{ __('Confirm Payment') }}</span>
                </button>
            </div>
        </form>
    </div>
