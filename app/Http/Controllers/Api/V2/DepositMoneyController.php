...
class DepositMoneyController extends Controller
{
    ...
    {
        try {
            $feesLimit = $this->service->findOrFailFeesDetails($request->currency_id, $request->payment_method_id);
            $wallet = $this->service->findOrFailWallet($request->currency_id);
            $this->service->amountIsInLimit($feesLimit, $request->amount);
            $totalAmount = $this->service->getTotalAmount($feesLimit, $request->amount);
    ...
    {
        try {
            // Fetch the fees limit
            $feesLimit = $this->service->findOrFailFeesDetails($request->currency_id, $request->payment_method_id);
            // find or create the wallet
            $wallet = $this->service->firstOrCreateWallet($request->currency_id);
            // check if the amount is in limit
            $this->service->amountIsInLimit($feesLimit, $request->amount);
            // Calculate the total amount
            $totalAmount = $this->service->getTotalAmount($feesLimit, $request->amount);
            // Pay through payment processor
            $processor->setPaymentType("deposit");
