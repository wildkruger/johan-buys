...

{
...

    {
         ...
         */
         ...
         if ($request->type == 'Profile payment') {
            if ($request->status == 'Success') {
                if ($t->status == 'Pending') {
                    DB::beginTransaction();
                    
                    $profilePayment         = \Modules\MerchantPayLink\Entities\ProfilePayment::find($request->transaction_reference_id);
                    $profilePayment->status = $request->status;
                    $profilePayment->save();
                    ...
                    $receiverWallet = Wallet::firstOrCreate(
                        ['user_id' => $request->user_id, 'currency_id' => $request->currency_id],
                        ['balance' => 0, 'is_default' => 'No']
                    );
    
                    $receiverWallet->increment('balance', $request->subtotal);
                    DB::commit();
    
                    $this->helper->one_time_message('success', __('The :x has been successfully updated.', ['x' => __('Profile payment')]));
                    return redirect(config('adminPrefix').'/transactions');
                } else {
                    DB::rollback();
                    $this->helper->one_time_message('error', __('The :x has already been successful.', ['x' => __('Profile payment')]));
                    return redirect(config('adminPrefix').'/transactions');
                }
            } else {
                DB::rollback();
                $this->helper->one_time_message('error', __('The :x has been Cancelled.', ['x' => __('Profile payment')]));
                return redirect(config('adminPrefix').'/transactions');
            }
        }
    }
