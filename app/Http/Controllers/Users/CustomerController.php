...

{
...

    {
        ...
        $data['publicUrl'] = generatePublicUrl($data['user']);

        $qrCode = QrCode::where(['object_id' => auth()->user()->id, 'object_type' => 'publicUrl', 'status' => 'Active'])->first(['secret']);

        if (empty($qrCode)){
            $publicUrl = generatePublicUrl($data['user']);

            $data['user']->profile_paylink_code = generateUniqueToken($data['user']);
            $data['user']->save();
            
            (new QrCode())->updateOrCreateQrCode($data['user'], 'publicUrl', $publicUrl);

            $qrCode = QrCode::where(['object_id' => auth()->user()->id, 'object_type' => 'publicUrl', 'status' => 'Active'])->first(['secret']);
        }

        
        $data['qrCodeSecret'] = $qrCode->secret;
    ...
    {
        $qrCodeSecret = convert_string('decrypt', $request->secret);
        $url = 'https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=' . urlencode($qrCodeSecret);
...

    {
        ...
        //unset($_COOKIE['paygate_deposit_user_id']);
