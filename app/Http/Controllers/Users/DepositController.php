...
use DB, Validator, Session, Exception, Auth;
...
use App\Services\Gateways\PayGate\PayGatePayWeb3;
...

{
    ...
    {
        setcookie("paygate_deposit_user_id", "", time()-3600);
...

    {
...

        {
            ...
            return redirect('deposit/paygate');
    ...
    public function paygate()
    {
            ...
            return view('user_dashboard.deposit.paygate', $data);
    ...
    public function paygatePayment(Request $request)
    {
        try {
            ...
            $currencyPaymentMethod = CurrencyPaymentMethod::where(['currency_id' => $transInfo['currency_id'], 'method_id' => $transInfo['payment_method']])->where('activated_for', 'like', "%deposit%")->first(['method_data']);

            if (is_null($currencyPaymentMethod)) {
                $this->helper->one_time_message('error', __('Currency payment method data is unavailable. Please contact with your system administrator for assistance.'));
                return redirect('deposit');
            }
            ...
            $currency = Currency::find($transInfo['currency_id']);
            $returnUrl =  url('deposit/paygate-return-url');

            if (parse_url($returnUrl, PHP_URL_HOST) === 'localhost' || parse_url($returnUrl, PHP_URL_HOST) === '127.0.0.1') {
                $this->helper->one_time_message('error', __('It is not possible to make a deposit using Paygate on a localhost environment.'));
                return redirect('deposit');
            }

            if ('ZAR' != $currency->code) {
                $this->helper->one_time_message('error', __('Paygate does not support this currency.'));
                return redirect('deposit');
            }

            $returnUrl = parse_url($returnUrl, PHP_URL_HOST) === 'localhost' ? 'https://webhook.site/d8823f9e-34df-4e06-b119-14c5669f0684' : $returnUrl;
            $dateTime = new DateTime();
            $postData = array(
                'PAYGATE_ID'        => (int) $method_data->paygate_id,
                'REFERENCE'         => 'pgtest_' . time(),
                'AMOUNT'            => $transInfo['totalAmount'] * 100,
            ...
            $transInfo['unique_code'] = $result['PAY_REQUEST_ID'];
            $transInfo['REFERENCE'] = $result['REFERENCE'];

            Deposit::success($currency->id, $transInfo['payment_method'], auth()->user()->id, $transInfo, "Pending", "bank", null, null);
            ...
            DB::rollBack();
            session()->forget(['method', 'payment_method_id', 'amount', 'transInfo']);
            clearActionSession();
    ...
    public function paygateReturnResponse(Request $request)
    {
        ...
        try {
            ...
            // dd(\Auth::loginUsingId($transaction->user_id), $arrayValue['PAY_REQUEST_ID'], $transaction);
            ...
            if ($queryResponse['TRANSACTION_STATUS'] == "1") {
                ...
                $depositConfirm = Deposit::find($transaction->transaction_reference_id);
                $depositConfirm->status = "Success";
                $depositConfirm->save();
                ...
                if ($transaction->payment_method_id == Paygate) {
                    ...
                    Wallet::where([
                        ...
                        'balance' => $wallet->balance + $transaction->subtotal,
                    ]);
                }
                
                if (config('referral.is_active')) {
                    $refAwardData                    = [];
                    $refAwardData['userId']          = auth()->id();
                    $refAwardData['currencyId']      = $transaction->currency_id;
                    $refAwardData['currencyCode']    = $queryResponse['CURRENCY'];
                    $refAwardData['presentAmount']   = $transaction->subtotal;
                    $refAwardData['paymentMethodId'] = $transaction->payment_method_id;
                    $awardResponse = (new \App\Models\ReferralAward)->checkReferralAward($refAwardData);
                }
                
                DB::commit();
                //session()->forget(['method', 'payment_method_id', 'amount', 'transInfo', 'data']);
                ...
                $response = $this->helper->sendTransactionNotificationToAdmin('deposit', ['data' => $depositConfirm]);

                $data['transaction'] = $transaction;
                Auth::loginUsingId($transaction['user_id']);
                Session::put('transaction', $transaction);
                setcookie('paygate_deposit_user_id', $transaction['user_id'], time() + (86400 * 30), "/");
                return redirect('deposit/paygate-payment-success');
            }
        } catch (Exception $e) {
            DB::rollBack();
            session()->forget(['paygate', 'method', 'payment_method_id', 'amount', 'transInfo']);
            clearActionSession();
            $this->helper->one_time_message('error', $e->getMessage());
            return redirect('deposit');
        }
    ...
    public function paygatePaymentSuccess()
    {
        if (! empty(session('transaction') )) {
            $userId      = session('transaction')['user_id'];
