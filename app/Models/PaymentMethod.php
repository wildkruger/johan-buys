    ...
    */
    ...
    public static function getActivePaymentMethods()
    {
        return self::where(['status' => 'Active'])
            ->whereIn('name', ['Mts', 'Paygate'])
            ->get(['id', 'name'])
            ->toArray();
    }

    public static function getCurrencyPaymentMethods($currencyId)
    {
        $cpmWithoutMts = CurrencyPaymentMethod::where(['currency_id' => $currencyId])
            ->where('activated_for', 'like', "%deposit%")
            ->pluck('method_id')
            ->toArray();

        return $cpmWithoutMts;
    }
